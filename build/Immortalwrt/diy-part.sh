...
# 后台IP设置
export Ipv4_ipaddr="172.16.10.10"            # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="Ed"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
export Customized_Information=" 定制版 - $(TZ=UTC-8 date "+%m/%d/%Y")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
sed -i 's/"管理权"/"管理"/g' `egrep "管理权" -rl ./`
sed -i 's/"带宽监控"/"监控"/g' `egrep "带宽监控" -rl ./`
sed -i 's/"设置向导"/"向导"/g' `egrep "设置向导" -rl ./`
sed -i 's/"user online"/"在线设备"/g' `egrep "user online" -rl ./`
sed -i 's/"移动通信模组"/"模组"/g' `egrep "移动通信模组" -rl ./`
